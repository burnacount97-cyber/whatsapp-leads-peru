...
export function SalesWidget() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: 'assistant', content: '👋 ¡Hola! Soy la IA de LeadWidget. Estoy aquí para ayudarte a convertir más visitas en clientes. ¿En qué te puedo ayudar hoy?' }
    ]);
    ...
    useEffect(() => {
        // Stop if user started typing, widget is closed, or we are loading, or they already sent a message
    ...
    const handleClose = () => {
        ...
        const randomMsg = TEASER_MESSAGES[Math.floor(Math.random() * TEASER_MESSAGES.length)];
        setActiveTeaser(randomMsg);
    };

    if (!isOpen) {
        return (
            <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 translate-y-0 animate-in slide-in-from-bottom-5 duration-500">
                {hasBeenClosedOnce && (
                    <div
                ...
                <button
                    ...
                    className="w-16 h-16 bg-[#00C185] rounded-full shadow-lg hover:scale-110 transition-transform flex items-center justify-center relative group"
                >
    ...
    return (
        <div className={`fixed bottom-4 right-4 left-4 sm:left-auto sm:bottom-24 sm:right-6 w-auto sm:w-[360px] h-[75vh] sm:h-[500px] bg-white rounded-2xl shadow-2xl z-50 overflow-hidden border border-border flex flex-col font-sans animate-in slide-in-from-bottom-5 duration-300 ${isIdle ? 'animate-vibrate-subtle' : ''}`}>
            {/* Header */}
            <div className="bg-[#00C185] p-4 flex items-center justify-between text-white shadow-md">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center relative">
                        <Bot className="w-6 h-6" />
                        <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 border-2 border-[#00C185] rounded-full"></div>
                    </div>
                    <div>
                        <h3 className="font-bold text-sm">IA LeadWidget</h3>
            ...
            <div className="flex-1 bg-slate-50 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
                        ...
                        {msg.role === 'system' ? (
                            ...
                            <div
                                className={`max-w-[85%] p-3 rounded-2xl text-sm shadow-sm ${msg.role === 'user'
                                    ? 'bg-[#00C185] text-white rounded-br-none'
                ...
                {/* Fake interaction hint: assistant is typing even if no input yet to grab attention */}
                    ...
                    <div className="flex justify-start animate-pulse-subtle">
                        <div className="bg-white p-3 rounded-2xl rounded-bl-none border border-slate-100 shadow-sm flex items-center gap-2">
                            <div className="flex gap-1">
                                <span className="w-1.5 h-1.5 bg-[#00C185] rounded-full animate-bounce delay-0"></span>
                                <span className="w-1.5 h-1.5 bg-[#00C185] rounded-full animate-bounce delay-150"></span>
                                <span className="w-1.5 h-1.5 bg-[#00C185] rounded-full animate-bounce delay-300"></span>
                            </div>
            ...
            <div className="p-4 bg-white border-t border-slate-100 space-y-3">
                ...
                <div className="text-center flex justify-center items-center gap-1">
                    <div className="w-1 h-1 bg-[#00C185] rounded-full"></div>
